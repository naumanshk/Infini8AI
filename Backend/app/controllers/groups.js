var Groups_schema = require('../models/groups_model')
...
var Groups_controller = {
    ...
    add_GroupsNotExist: (req, res,next) => {
        ...
        Groups_schema.find({
            ...
            if (doc.length==0 ) {
                res.status(200);
                var Groups = new Groups_schema(req.body);
    ...
    addMembers: (req, res) => {
        ...
        Groups_schema.update({ name: req.params.id }, { $push: { members: req.body } }, { new: true }, (err, todo) => {
    ...
    findByMember: (req, res, next) => {
        console.log(req.params.id)

        Groups_schema.find({'members.user.email':req.params.id}).exec(function (err, doc) {
